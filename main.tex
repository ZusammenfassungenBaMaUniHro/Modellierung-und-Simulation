...
\section{Hinweis}
Die Urheberrechte der Bilder liegen bei Prof. Adelinde Uhrmacher, entstammen der Vorlesung "Modellierung und Simulation" aus dem Sommer Semester 2014. Die Texte sind an die Vorlesung angelehnt und entsprechen teilweise dieser. In diesem Fall liegen die Urheberrechte ebenfalls bei Prof. Adelinde Uhrmacher. Die Autoren übernehmen keine Garantie für Richtigkeit und Vollständigkeit.
\section{Einführung}
...
\section{Stochastik}
	...
	\end{itemize}
	\begin{center}
		 \includegraphics[scale=0.5]{images/uniform.jpg}
	\end{center}
	...
	\end{itemize}
	\begin{center}
		 \includegraphics[scale=0.5]{images/u01.jpg}
	\end{center}
	...
	\end{itemize}
	\begin{center}
		 \includegraphics[scale=0.5]{images/exp.jpg}
	\end{center}
	...
	\end{itemize}
	\begin{center}
		 \includegraphics[scale=0.5]{images/norm.jpg}
	\end{center}
	\subsubsection{Possionverteilung}
	\begin{itemize}
		\item $f(x) = \left\{ 
			\begin{array}{ l l}
	...
	\end{itemize}
	\begin{center}
		 \includegraphics[scale=0.5]{images/poisson.jpg}
	\end{center}
	\subsection{Bestimmung von Verteilungen}	
		Nachfolgend interessiert uns die Frage wie wir von Beobachtungen zu der dazugehörigen Wahrscheinlichkeitsverteilung kommen. Häufig werden dafür explizite Annahmen getroffen. Durch ein Vergleich von der Erwartung und dem erzielten Ergebnis erhalten wir eine akzeptable Verteilung. Durch Tests wird anschließend überprüft ob  wie gut die Verteilung zu den Beobachtungen passt. Beispiel : $\chi^{2}  = \sum_{j=i}^{k} \frac{(N_{j}-n*p_{j})^{2}}{n*p_{j}}$.
	\subsection{Zufallszahlen}
		Ziel von Pseudo-Zufallszahlengeneratoren ist es Zufallszahlen die uniform verteilt sind zu berechnen. Dabei werden Zufallszahlen durch eine feste Formel errechnet. Für die Reproduzierbarkeit von Experimenten ist es notwendig ausgehend von einem Seed die Sequenz von Zufallszahlen Steuern zu können $\rightarrow$  Seed reproduziert immer die gleiche Sequenz an Zufallszahlen. Probleme dabei sind jedoch das die generierten Zufallszahlen nicht zufällig sind und die Berechnung eine Periode (Wiederholung der Sequenz) aufweisen. Ziel ist es  eine möglichst große Periode zu erreichen. Bekannte Zufallszahlengeneratoren sind: Mersenne Twister, LCG. Häufig ist es jedoch gewünscht Zufallszahlen einer anderen Verteilung als $U(0,1)$ zu generieren. Eine mögliche Technik ist die Inverse Transformationsmethode. Bei dieser gehen wir davon aus das wir zu einer gewünschten Verteilungsfunktion die Inverse $F^{-1}$ berechnen können. Wir erhalten Zufallszahlen $X$ der Verteilung $F$ durch die Berechnung von $X = F^{-1}(U)$ mit U ist gleichverteilt im Intervall von $[0,1)$.
	\subsection{Stochastik in Experimenten}
		Bei stochastischen Modellen ist es notwendig mehere Läufe der Simulation zu berechnen. Ein einzelner Lauf erlaubt keine allgemeingültigen Aussagen. Eine sehr wichtige Frage in diesem Zusammenhang ist es die Anzahl der Replikationen zu bestimmen. Ziel ist es dabei nicht zu viele und nicht zu wenig Replikationen zu berechnen d.h. wir wünschen eine Interpretation mit einer gewünschten Sicherheit. Dies wird mittels Kofidenzintervallen ausgedrückt. Ein Kofidenzintervall $P(u_{1}\leq \mu \leq u_{2}) = \gamma$ gibt die Wahrscheinlichkeit an, dass der Kennwert $\gamma$ zwischen $u_{1}$ und $u_{2}$ liegt, $\mu$ beträgt.Es kann also mit einer Wahrscheinlichkeit $\mu$ garantiert werden, dass der Kennwert $\gamma$ in $[u_{1},u_{2}]$ liegt. Um die Länge eines Lauf zu steuern gibt es einige Möglichkeiten:
			\begin{itemize}
				\item Absoluter Zeitwert überschritten
				\item Anzahl der Ereignisse reicht nicht mehr aus
				\item Bestimmte Charakteristiken der berechneten Daten sind erreicht (Oszillilation, Schwellenwerte)
			\end{itemize}
		Dabei ist zu berücksichtigen dass bestimmte Modelle eine Warm-Up-Phase aufweisen. Die Warm-Up-Phase ist dabei eine Phase in dem sich das modellierte System erst in einen Bestimmten zustand einschwingt. Beispiel: Wenn wir die Mensa im Vollbetrieb optimieren möchten, ist es nicht von Interesse die ersten paar Minuten der Öffnung zu analysieren.
		\begin{center}
			\includegraphics[scale=0.5]{images/experiment.jpg}
		\end{center}
\section{Kontinuierliche Modelle}
	...
	\subsection{Modellierung und Simulation}
		...
		Bezüglich der Parameter gibt es hierbei zwei verschiedene Fälle:
			...
			\end{enumerate}
		Für die Berechnung dieser Modelle sind numerische Integrationsverfahren notwendig.
	...
	\subsection{Blockdiagramme}
		Blockdiagramme sind die graphische Repräsentation von Differentialgleichungen in Simulink, wobei das zentrale Element der Integratorblock ist. Die Ideen dabei ist das jede Variable als Block dargestellt wird. Die Konstanten der Variablen werden durch ein Pfeil zum Block dargestellt.
